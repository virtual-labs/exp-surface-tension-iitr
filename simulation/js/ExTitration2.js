...
const knob_click = new Audio('songs/rachetOneCut.mp3');
...
// Retrieve ranWeight and ranDrop from localStorage if they exist
let storedRanWeight = localStorage.getItem('ranWeight');
let storedRanDrop = localStorage.getItem('ranDrop');

if (storedRanWeight !== null) {
  ranWeight = JSON.parse(storedRanWeight);
}
if (storedRanDrop !== null) {
  ranDrop = JSON.parse(storedRanDrop);
}

console.log('Data retrieved from localStorage:', { ranWeight, ranDrop });
...
function preload() {
  ...
  beaker=loadImage('images/PotasiumChromate.png');
...
function nextpressed() {
  ...
  localStorage.setItem('ranWeight2', JSON.stringify(ranWeight));
  localStorage.setItem('ranDrop2', JSON.stringify(ranDrop));
  
  console.log('Data stored in localStorage:', { ranWeight, ranDrop });
  
  // Redirect to the results/completion page
